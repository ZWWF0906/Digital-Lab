...
def _html_report(img_cpu: str, img_mem: str, img_disk: str,
    ...
    html.append("<title>DigitalLab 监控报告</title>")
    html.append("<style>")
    ...
    html.append("<h1>DigitalLab 监控报告</h1>")
    ...
    html.append('<div class=footer>DigitalLab 自动生成 — {}</div>'.format(now))
...
def format_terminal_summary(result: dict) -> str:
    ...
    lines.append("")
    lines.append("=" * 60)
    lines.append("  DigitalLab 分析报告 — {}".format(label))
    lines.append("=" * 60)
