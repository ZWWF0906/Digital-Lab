...
_CSS = r"""
/* ============================================================
   DigitalLab Design System — Emerald Dark
   ============================================================ */
:root{
/* foundation */
--bg-primary:#0a0e14;
--bg-secondary:rgba(255,255,255,.025);
--bg-tertiary:rgba(255,255,255,.05);
--bg-elevated:rgba(14,19,26,.92);
/* accent */
--accent:#00e5a0;
--accent-bright:#00c9ff;
--accent-teal:#00e5a0;
--accent-purple:#a78bfa;
--accent-pink:#f43f5e;
/* text */
--text-primary:#e8ecf1;
--text-secondary:rgba(232,236,241,.55);
--text-tertiary:rgba(232,236,241,.25);
/* border */
--border:rgba(0,229,160,.06);
--border-hover:rgba(0,229,160,.2);
/* glow - single layer, hover only */
--glow-hover:rgba(0,229,160,.1);
--glow-inner-hover:rgba(0,229,160,.03);
/* layout */
--radius-sm:8px;--radius-md:16px;--radius-lg:24px;
--gap-sm:12px;--gap-md:20px;--gap-lg:28px;--padding-card:24px;
...
--accent:#059669;--accent-bright:#0284c7;
--accent-teal:#059669;--accent-purple:#7c3aed;--accent-pink:#dc2626;
--text-primary:#111827;
--text-secondary:rgba(17,24,39,.55);
--text-tertiary:rgba(17,24,39,.25);
--border:rgba(5,150,105,.12);
--border-hover:rgba(5,150,105,.3);
--glow-hover:rgba(5,150,105,.06);
--glow-inner-hover:rgba(5,150,105,.02)
}
...
.side a.active{background:rgba(0,229,160,.1);color:var(--accent-teal);font-weight:500}
.side a.active::before{content:'';position:absolute;left:0;top:6px;bottom:6px;width:3px;background:var(--accent-teal);border-radius:0 4px 4px 0;box-shadow:0 0 8px var(--accent-teal)}
...
.main{margin-left:252px;margin-top:56px;padding:28px 32px 60px}
...
th{text-align:left;padding:11px 16px;border-bottom:1px solid var(--border);font-size:10px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.06em}
td{padding:10px 16px;border-bottom:1px solid var(--border);font-size:13px;font-weight:400}
td:nth-child(n+3){font-family:var(--font-mono);font-feature-settings:'tnum';font-variant-numeric:tabular-nums;text-align:right}
tr:nth-child(even) td{background:rgba(255,255,255,.012)}
tr:hover td{background:var(--bg-tertiary)}
.panel{display:none}.panel.on{display:block}

/* ---- buttons ---- */
.btn{display:inline-flex;align-items:center;justify-content:center;padding:8px 20px;border-radius:var(--radius-sm);border:1px solid var(--border);font:500 13px var(--font-sans);cursor:pointer;transition:transform var(--dur-fast) var(--ease-smooth),background var(--dur-normal) var(--ease-smooth),box-shadow var(--dur-fast) var(--ease-smooth);color:var(--text-primary);background:var(--bg-tertiary)}
...
.stat-item{background:var(--bg-secondary);border:1px solid var(--border);border-radius:var(--radius-md);padding:16px 20px;min-width:120px;transition:transform var(--dur-fast) var(--ease-smooth),border-color var(--dur-normal) var(--ease-smooth)}
...
var RING_COLORS=['var(--accent-teal)','var(--accent-bright)','var(--accent-purple)'];
...
function loadPanel(id){
    panel(id);CP=id;
    var m={'home':H,'processes':P,'alerts':AL,'init':I,'status':ST,'config-panel':CF,'daemon':D,'alert-test':AT,'compare':CMP,'launcher':LA,'report':RP,'timeline':TL};
    if(m[id])m[id]()
}
...
/* ---- ring color: green/cyan by load, pink at high ---- */
function ringClr(key,val){
    if(val>=80)return'var(--accent-pink)';
    if(key==='cpu')return'var(--accent-teal)';
    if(key==='memory')return'var(--accent-bright)';
...
function HW(){
    api('/hardware').then(function(d){
        ...
        if(xnet&&xnet.adapters&&xnet.adapters.length){
            ...
            var naSub=[];
            if(na.name)naSub.push(na.name);
            if(na.speed_mbps)naSub.push(na.speed_mbps+' Mbps');
            if(xnet.adapters.length>1)naSub.push('+ '+(xnet.adapters.length-1)+' adapters');
            var netMain=(na.type||'')+(na.active?' \u2022':'');
            if(!netMain)netMain='Network Adapter';
            cp={key:'network',title:'NETWORK',icon:'\u2601',main:netMain,sub:naSub.join(' · ')};
        }else{
        ...
        for(var i=0;i<3;i++){
            ...
            if(el){
                ...
                if(numSpan){
                    ...
                    el.style.color=clr;
                    numAnim(numSpan,cur,val,80,function(v){return Math.round(v)})
                }
...
function H(){
    ...
    for(var i=0;i<3;i++){
        ...
        s+='<div class="val" id="rv-'+key+'" style="color:var(--text-primary)"><span class="val-num" id="vn-'+key+'">--</span><span class="val-unit">%</span></div>';
        s+='</div>';
...
/* ---- alerts ---- */
function AL(){api('/alerts').then(function(d){if(!d||!Array.isArray(d)||!d.length){showEmpty('alerts','No alerts');return}var h='<table><tr><th>Time</th><th>Metric</th><th>Value</th><th>Threshold</th></tr>';for(var i=0;i<d.length;i++){var a=d[i],dt=a.data||{};h+='<tr><td>'+a.timestamp+'</td><td>'+a.message+'</td><td>'+(dt.current||0).toFixed(1)+'%</td><td>'+dt.threshold+'%</td></tr>'}h+='</table>';$('p-alerts').innerHTML=h}).catch(function(){showEmpty('alerts')})}function ALY(){api('/timeline?range=year').then(function(d){if(!d||d.error)return;var h='<h2>Year Review  '+d.year+'</h2>';h+='<div class=row>';h+='<div class=item><div class=lbl>Active Days</div><div class=num style=color:var(--accent-teal)>'+d.active_days+'</div></div>';h+='<div class=item><div class=lbl>Avg Active</div><div class=num>'+d.avg_active_hours+'h</div></div>';h+='<div class=item><div class=lbl>Avg CPU</div><div class=num>'+d.avg_cpu+'%</div></div>';h+='<div class=item><div class=lbl>Avg Mem</div><div class=num>'+d.avg_memory+'%</div></div>';h+='<div class=item><div class=lbl>Peak CPU</div><div class=num style=color:var(--accent-pink)>'+d.peak_cpu+'%</div></div>';h+='<div class=item><div class=lbl>Peak Mem</div><div class=num style=color:var(--accent-pink)>'+d.peak_memory+'%</div></div>';h+='<div class=item><div class=lbl>Events</div><div class=num>'+d.total_events+'</div></div>';h+='<div class=item><div class=lbl>Snaps</div><div class=num>'+d.total_snapshots+'</div></div>';h+='</div>';if(d.top_apps&&d.top_apps.length){h+='<h3 style=margin-top:20px>Top Apps</h3><table><tr><th>#</th><th>App</th><th>Days</th></tr>';for(var i=0;i<d.top_apps.length;i++){h+='<tr><td>'+(i+1)+'</td><td>'+d.top_apps[i].name+'</td><td>'+d.top_apps[i].count+'</td></tr>'}h+='</table>'}var yr=$('tl-year');if(yr)yr.innerHTML=h}).catch(function(){})}

/* ---- timeline ---- */
var _tl_range='today';
function TL(){
    var h='<h2>Timeline</h2><p class=hint>Daily activity tracking & year review.</p>';
    h+='<div class=toolbar><div class=seg id=seg-tl><div class=seg-ind></div>';
    h+='<span class=on onclick=\"segClick(\"seg-tl\",\"today\",this,function(v){_tl_range=v;TLL()})\">Today</span>';
    h+='<span onclick=\"segClick(\"seg-tl\",\"7d\",this,function(v){_tl_range=v;TLL()})\">7d</span>';
    h+='<span onclick=\"segClick(\"seg-tl\",\"30d\",this,function(v){_tl_range=v;TLL()})\">30d</span>';
    h+='<span onclick=\"segClick(\"seg-tl\",\"year\",this,function(v){ALY();return})">Year</span>';
    h+='</div></div><div id=tl-list></div><div id=tl-year style=margin-top:24px></div>';
    $('p-timeline').innerHTML=h;initSegIndicators();TLL()
}
function TLL(){
    if(_tl_range==='year'){ALY();return}
    api('/timeline?range='+_tl_range).then(function(d){
        var c=$('tl-list');if(!c)return;
        if(!d.timeline||!d.timeline.length){c.innerHTML='<div class=empty><h3>No daily data yet</h3><p>Data is recorded every 3 minutes. Come back later.</p></div>';return}
        var s=d.stats||{};
        var h='<div class=msg style=margin-bottom:12px>';
        h+='Days: '+s.total_days+' | Active: '+s.avg_active_hours+'h | CPU: '+s.avg_cpu+'% | Mem: '+s.avg_memory+'%';
        h+='</div>';
        h+='<table><tr><th>Date</th><th>Active</th><th>CPU</th><th>Mem</th><th>Events</th><th>Snaps</th><th>Top App</th></tr>';
        for(var i=d.timeline.length-1;i>=0;i--){
            var e=d.timeline[i];
            h+='<tr><td>'+e.date+'</td><td>'+e.active_hours+'h</td><td>'+e.cpu_avg+'%</td><td>'+e.memory_avg+'%</td><td>'+e.event_count+'</td><td>'+e.snapshot_count+'</td><td style=font-size:12px;color:var(--text-tertiary)>'+(e.top_processes||[]).slice(0,2).join(', ')+'</td></tr>'
        }
        h+='</table>';c.innerHTML=h
    }).catch(function(){var c=$('tl-list');if(c)c.innerHTML='<div class=empty><h3>Timeline not available</h3></div>'})
}
...
/* ---- theme toggle ---- */
 ...
 document.addEventListener('DOMContentLoaded',function(){
     ...
     H();
    setInterval(function(){try{RF()}catch(e){}},1000);
...
def _get_auth_token():
    # 1. 环境变量（最高优先级）
    token = os.environ.get('DIGITAL_LAB_TOKEN', '').strip()
    if token:
        return token

    # 2. config.json 中的 auth_token
    try:
        ...
        if os.path.exists(_config_path):
            with open(_config_path, 'r', encoding='utf-8') as _f:
                _cfg = _json.load(_f)
            token = _cfg.get('auth_token', '').strip()
            if token:
                return token
    except Exception:
        pass

    # 3. 自动生成，并写入 config.json
    ...
    try:
        ...
        if os.path.exists(_config_path):
            ...
            _cfg = {}
        _cfg['auth_token'] = token
        with open(_config_path, 'w', encoding='utf-8') as _f:
            _json.dump(_cfg, _f, indent=2, ensure_ascii=False)
        print(f"[auth] Token 已写入 {_config_path}")
    except Exception as e:
        print(f"[auth] 无法写入 config.json: {e}")

    return token

AUTH_TOKEN = _get_auth_token()
AUTH_ENABLED = os.environ.get('DIGITAL_LAB_AUTH_ENABLED', 'true').lower() in ('1', 'true', 'yes')
...
def require_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if request.args.get('token', '') == AUTH_TOKEN:
            return f(*args, **kwargs)
        return jsonify({'error': 'Unauthorized', 'message': '请提供有效的认证 Token'}), 401
...
def create_app() -> Flask:
    ...
    panels = ["home", "processes", "alerts", "init", "status", "config-panel",
              ...
              "timeline", "experiment", "analyze", "ai"]
    panels_html = "\n".join(
    ...
    def index():
        ...
        html += '<title>DigitalLab</title>\n'
        html += '<style>\n' + _CSS + '\n</style>\n'
        html += '</head>\n<body>\n'
        html += _build_sidebar()
        html += '\n<div class="main">\n' + panels_html
        html += '\n<div class="footer-cr"><span class="cr-name">DigitalLab</span> \u00A9 2026 ZWWF0906</div>'
        html += '\n</div>\n'
        html += _SCRIPTS
        html += '\n</body>\n</html>'
        return html
    ...
    def api_current():
        ...
        from core.renderer import render_web_current
        snap = system_state.snapshot()
        return jsonify(render_web_current(snap))
    ...
    def api_hardware():
        try:
            from core.system_state import system_state
            from core.renderer import render_web_hardware
            data = render_web_hardware(system_state.snapshot())
            return jsonify(data)
        except Exception:
    ...
    def api_processes():
        from core.system_state import system_state
        from core.renderer import render_web_processes
        result = render_web_processes(system_state.snapshot())
        if not result:
            result = render_web_processes(system_state.snapshot())
    ...
    def api_alerts():
        from core.system_state import system_state
        from core.renderer import render_web_alerts
        return jsonify(render_web_alerts(system_state.snapshot()))
    ...
    def api_status():
        try:
            from core.system_state import system_state
            from core.renderer import render_web_status
            return jsonify(render_web_status(system_state.snapshot()))
        except Exception as e:
    ...
    @app.route("/api/data")
    @require_auth
    def api_data_compare():
        rng = request.args.get("range", "24h")
        seconds = {"24h": 86400, "7d": 604800}.get(rng, 86400)
        cutoff = (datetime.datetime.now() - datetime.timedelta(seconds=seconds)).isoformat()
        from core.config import get_config
        db_path = get_config().monitor_db_path
        times, cpu_vals, mem_vals, disk_vals = [], [], [], []
        try:
            ...
            for ts, cpu, mem, disk in cur:
                times.append(ts)
                cpu_vals.append(round(cpu, 1))
                mem_vals.append(round(mem, 1))
                disk_vals.append(round(disk, 1))
            conn.close()
    ...
    def api_timeline():
        ...
        try:
            from core.memory import load_timeline, generate_year_review
            if rng == "year":
                return jsonify(generate_year_review())
            days = {"today": 1, "7d": 7, "30d": 30}.get(rng, 7)
            return jsonify(load_timeline(days))
        except Exception:
            return jsonify({"error": "timeline unavailable", "timeline": []})
...
def start_dashboard(host="127.0.0.1", port=8080, daemon_thread=True):
    ...
    if AUTH_ENABLED:
        print(f"\n{'='*50}")
        print(f"  仪表盘访问 Token: {AUTH_TOKEN}")
        print(f"  请使用以下 URL 访问:")
        print(f"  http://{host}:{port}?token={AUTH_TOKEN}")
        print(f"{'='*50}\n")

    return "[OK] Dashboard 已启动: http://{}:{}".format(host, port)
if __name__ == '__main__':
    print(start_dashboard(host='0.0.0.0', port=8080, daemon_thread=False))