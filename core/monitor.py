...
def format_report() -> str:
    ...
    lines.append("=" * 60)
    lines.append("  DigitalLab - 系统监控报告")
...
def _get_pending_queue():
    from core.system_state import system_state
    m = system_state["monitor"]
    d = m.get("db", {})
    if not d:
        d = {"pending": []}
        system_state["monitor"]["db"] = d
    return d["pending"]


def _set_pending_queue(value: list):
    from core.system_state import system_state
    m = system_state["monitor"]
    d = m.get("db", {"pending": []})
    d["pending"] = value
...
def insert_snapshot(cpu: float, memory: float, disk: float):
    ...
    with _pending_lock:
        _get_pending_queue().append(entry)
...
def collect_and_store():
    ...
    now_ts = time.time()
    from core.system_state import system_state, _upsert_with_ts
    _upsert_with_ts("monitor", "cpu", cpu_val, now_ts)
    _upsert_with_ts("monitor", "memory", mem_val, now_ts)
    _upsert_with_ts("monitor", "disk", disk_val, now_ts)
    system_state["monitor"]["timestamp"] = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S")

    from core.event_bus import emit_batch
    emit_batch("monitor", {
        "cpu": cpu_val,
        "memory": mem_val,
        "disk": disk_val,
    ...
    try:
        ...
        system_state["monitor"]["processes"] = procs
    except Exception:
        pass

    # 网络速度（Mbps）— 基于两次采集间的字节增量计算
    global _prev_net_io, _prev_net_time
    try:
        net_io = psutil.net_io_counters()
        total_bytes = net_io.bytes_sent + net_io.bytes_recv
        if _prev_net_io is not None and _prev_net_time is not None:
            delta_bytes = total_bytes - _prev_net_io
            delta_sec = now_ts - _prev_net_time
            if delta_sec > 0:
                speed_mbps = round((delta_bytes * 8) / (delta_sec * 1_000_000), 1)
                system_state["monitor"]["network_speed"] = speed_mbps
        _prev_net_io = total_bytes
...
def format_live(clr: bool = False) -> str:
    ...
    lines.append("  DigitalLab 实时监控  |  {}  |  Ctrl+C 退出".format(now))
    lines.append("-" * 60)
