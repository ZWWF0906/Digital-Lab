...
def _detect_active_interface_alias() -> Optional[str]:
    """Detect the active internet connection interface alias on Windows.
    Returns the primary active interface alias (excluding VPN adapters).
    """
    if sys.platform != "win32":
        return None
    # Method 1: Get-NetConnectionProfile (most reliable for active connection)
    out = _run_ps(
        "Get-NetConnectionProfile | "
        "Select-Object -ExpandProperty InterfaceAlias"
    )
    if out:
        aliases = [a.strip() for a in out.splitlines() if a.strip()]
        vpn_keywords = ["radmin", "vpn", "zerotier", "tailscale", "wireguard",
                        "openvpn", "nord", "express", "surfshark", "proton"]
        # Prefer non-VPN adapters
        for alias in aliases:
            is_vpn = any(kw in alias.lower() for kw in vpn_keywords)
            if not is_vpn:
                return alias
        # Fallback to first alias if all are VPN
        if aliases:
            return aliases[0]
        return None
    # Method 2: Get-NetRoute for default route (fallback)
    out = _run_ps(
...
def get_network_info() -> dict:
    result = {"adapters": [], "active_type": None}
    if sys.platform == "win32":
        active_alias = _detect_active_interface_alias()
        out = _run_ps(
            "Get-CimInstance Win32_NetworkAdapter | "
            "Where-Object {$_.NetEnabled -eq $true -and $_.PhysicalAdapter -eq $true} | "
            "Select-Object Name,NetConnectionID,AdapterType,Speed,Manufacturer,PNPDeviceID,GUID | ConvertTo-Json"
        )
        if out:
            try:
            ...
            for a in adapters_raw:
                ...
                netconn_id = (a.get("NetConnectionID") or "").strip()
                ...
                # Detect active interface: match NetConnectionID against active alias
                is_active = False
                if active_alias and netconn_id:
                    is_active = (netconn_id.lower() == active_alias.lower())
                adapter = {
                    "name": name_clean,
                    "type": adapter_type,
                ...
                if is_active:
                    active_list.append(adapter)
                    result["active_type"] = adapter_type
                elif is_wireless:
            ...
            # Sort: active first, then wireless, then wired, then other
            result["adapters"] = active_list + wireless + wired + other
        return result
    if HAS_PSUTIL:
        try:
...
def collect_all(use_cache: bool = True) -> dict:
    ...
    from core.system_state import system_state
    system_state["hardware"] = data

    return data


def refresh_hardware_live():
    """Re-query volatile hardware components (GPU, network, display)
    and update system_state['hardware'] in-place.

    This is called periodically by the collector to keep hardware
    data current without relying on the file cache.
    """
    from core.system_state import system_state
    ...
    # Re-query GPU (may change when switching between iGPU/dGPU)
    try:
        gpu = get_gpu_info()
        if gpu:
            hw["gpu"] = gpu
    ...
    # Re-query network (active interface may change)
    try:
        network = get_network_info()
        if network:
            hw["network"] = network
    ...
    # Re-query display (may change with external monitors)
    try:
        display = get_display_info()
        if display:
            hw["display"] = display
    ...
    # Re-run classifier on updated data
    try:
        from core.hardware_classifier import (
            classify_cpu_suffix,
            detect_device_type,
        ...
        hw["_validation"] = validation

        display_cfg = generate_display_config(hw)
        hw["_display"] = display_cfg
    except ImportError:
        pass

    # Write back to system_state
    system_state["hardware"] = hw
