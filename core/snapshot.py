...
def create_snapshot(note: str = "") -> dict:
    ...
    system_state["snapshot"]["last"] = snap
    return snap
...
def list_snapshots() -> list:
    ...
    for name in sorted(os.listdir(SNAP_DIR), reverse=True):
        ...
        try:
            ...
            continue
    from core.system_state import system_state
    system_state["snapshot"]["list"] = snaps
    return snaps
...
def delete_snapshot(snap_id: str) -> bool:
    ...
    if os.path.exists(path):
        ...
        from core.system_state import system_state
        system_state["snapshot"]["last_deleted"] = snap_id
        return True
    return False
...
def _load(sid):
    from core.system_state import system_state
    cache = system_state.get("snapshot", {}).get("cache", {})
    if sid in cache:
        return cache[sid]
    ...
    cache[sid] = snap
    system_state["snapshot"]["cache"] = cache
    return snap
